...
public:
  ...
  /// Start the matching engine thread (asynchronous mode).
  void start();

  /// Stop the matching engine thread.
  void stop();

  /// Push a command from the I/O thread to the engine.
  bool push_command(const ClientCommand& cmd) { return in_queue_.push(cmd); }

  /// Pop an event generated by the engine (for the I/O thread).
  bool pop_event(EngineEvent& ev) { return out_queue_.pop(ev); }
  ...
  /// Check if the engine has a pending/resting order with the given ID.
  [[nodiscard]] bool has_order(OrderId id) const { return book_.get_order(id) != nullptr; }
  ...
  [[nodiscard]] int64_t total_maker_rebates() const noexcept { return total_maker_rebates_; }
  [[nodiscard]] int64_t total_taker_fees() const noexcept { return total_taker_fees_; }
...
private:
  ...
  /// Process a popped client command and generate relevant events.
  void process_client_command(const ClientCommand& cmd);

  OrderBook& book_;
  FillCallback on_fill_;
  FeeSchedule fees_;

  SPSCQueue<ClientCommand> in_queue_{8192};
  SPSCQueue<EngineEvent> out_queue_{8192};
  std::thread engine_thread_;
  alignas(64) std::atomic<bool> engine_running_{false};

  uint64_t total_fills_ = 0;
  uint64_t total_orders_ = 0;
  uint64_t total_cancels_ = 0;
  uint64_t total_modifies_ = 0;
  int64_t total_maker_rebates_ = 0;
  int64_t total_taker_fees_ = 0;
};
